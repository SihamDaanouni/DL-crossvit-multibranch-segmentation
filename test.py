...
from src.dataset import SplitDataset  # Votre fichier dataset.py
from src.models.crossvit_general import CrossViTClassifier, CrossViTBackbone  # Votre fichier model.py
...
def main():
    ...
    configs = {
        ...
        "A": {"route": [0, 0], "same_resolution": False, "desc": "Non-segmented only"},
        "B": {"route": [1, 1], "same_resolution": False, "desc": "Segmented only"},
        "C1": {"route": [0, 1], "same_resolution": False, "desc": "Non-seg → Small, Seg → Large"},
        "C2": {"route": [1, 0], "same_resolution": False, "desc": "Seg → Small, Non-seg → Large"},

        # O2: Iso-résolution
        "O2": {"route": [0, 1], "same_resolution": True, "desc": "Iso-resolution (Small=Large)"},

        # O3: Pondération par patch (à activer dans le modèle)
        "O3": {"route": [0, 1], "same_resolution": True, "patch_weighting": True, "desc": "Iso-resolution + patch weighting"},
    ...
    model = CrossViTClassifier(
        ...
        num_classes=2,
        img_size=224,
        patch_weighting=config.get("patch_weighting", False) # À activer pour O3
    ).to(device)
