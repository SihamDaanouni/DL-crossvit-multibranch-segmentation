...
def compute_patch_weights(mask_tensor, patch_size=16, gamma=1.0, epsilon=0.01):
    """
    Calcule wp = (epsilon + rp)^gamma avec normalisation unitaire.
    rp est le ratio de pixels plante dans le patch.
    """
    # 1. Masque binaire : 1 si pixel > 0 (plante), 0 sinon
    binary_mask = (mask_tensor.mean(dim=1, keepdim=True) > 0.05).float()
    
    # 2. Calcul du ratio rp par patch (aligné avec la grille ViT) 
    ...
    # 4. Fonction puissance f(rp) 
    wp = torch.pow(epsilon + rp, gamma)
    
    # 5. Normalisation unitaire par image 
